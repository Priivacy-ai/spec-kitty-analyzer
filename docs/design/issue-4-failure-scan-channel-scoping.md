...
The only existing guard, `jsonLooksLikeSourceRead` (`fingerprints.go:395`), suppresses
just the Claude Read shape. The generic fallback already narrows to
...
- **File reads**: `toolUseResult.file.content` (today's `jsonLooksLikeSourceRead`,
...
- Generic standalone `.log`/`.txt` command-output files → **explicitly unsupported**
  for now; revisit by introducing a `command_log` source kind in a later change. This
  is documented, not silently mishandled.
...
`json_error_event`, `review_rejected` (`review_status`/`verdict` fields). These key off
specific JSON fields and are out of scope.
...
- **Compute and cache two strings once per event** — `outputText(obj)` and
  ...
  (Codex finding #6).
- Generalize `jsonLooksLikeSourceRead` into the §3a code-edit/file-read exclusion.
...
- **Ordering** (make explicit, with tests): (1) structural `obj` rules; (2) §3a
  ...
  (subject to the existing `artifactFailureAllowed` exceptions, e.g. `review_rejected`),
  drop its failures *and* anomalies before they reach `findings`/anomaly aggregation.
...
2. **`obj == nil` plain text** → resolved in §3d (artifact/spec → diagnostic-only;
   transcript text → output-eligible; generic `.log` unsupported, documented).
...
6. **Corpus FN/FP sweep** across all five harness shapes, main vs candidate binary,
   ...
   **down** and `branch_worktree_confusion` FP **unchanged**. 233 missions cached
